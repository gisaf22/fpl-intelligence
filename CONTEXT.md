...
| CLAUDE.md | `CLAUDE.md` | Standing rules ("never break these") + the session start-up read order; auto-loaded at session start | Every session — all code, study, and refactor work |
| CONTEXT.md | `CONTEXT.md` | Current project state, structure, and document hierarchy | New session orientation |
...
## 9. Rules and session start-up

Moved to `CLAUDE.md` at the repo root (§9 Rules, §10 How to start a new session) — it loads automatically at session start.
