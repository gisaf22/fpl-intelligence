...
def validate_bgw_correctness(df: pd.DataFrame, performance_cols: set[str] | list[str] | None = None) -> None:
    ...
    _raise_if(bgw, bgw['fixture_count'] != 0,
              "BGW correctness violation: have fixture_count != 0",
              "validate_bgw_correctness", "BGW_VIOLATION")

    for col in (performance_cols or ()):
        _raise_if_notnull(bgw, col,
                          f"BGW correctness violation: have {col} not null",
                          "validate_bgw_correctness", "BGW_VIOLATION")

    _raise_if_notnull(bgw, "fdr_avg",
                      "BGW correctness violation: have fdr_avg not null",
                      "validate_bgw_correctness", "BGW_VIOLATION")

    _raise_if_notnull(bgw, "was_home",
...
def validate_dgw_correctness(df: pd.DataFrame) -> None:
    ...
    _raise_if(dgw, dgw['fixture_count'] != 2,
              "DGW correctness violation: have fixture_count != 2",
              "validate_dgw_correctness", "DGW_VIOLATION")

    if 'home_count' in dgw.columns and 'away_count' in dgw.columns:
        _raise_if(dgw, (dgw['home_count'] + dgw['away_count']) != 2,
                  "DGW correctness violation: have home_count + away_count != 2",
                  "validate_dgw_correctness", "DGW_VIOLATION")

    _raise_if_null(dgw, "fdr_avg",
                   "DGW correctness violation: have fdr_avg null",
                   "validate_dgw_correctness", "DGW_VIOLATION")

    if 'clean_sheets' in dgw.columns:
        _raise_if(dgw, ~dgw['clean_sheets'].isin([0, 1, 2]),
