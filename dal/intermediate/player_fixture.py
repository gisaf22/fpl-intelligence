...
def get_player_fixture_base(db_path: Path, gw: int | None = None) -> pd.DataFrame:
    ...
    positions = get_staged_element_types(db_path)
    fixtures = get_fixture_context(db_path)
    result = _join_player_fixture(player_histories, players, positions, fixtures)
