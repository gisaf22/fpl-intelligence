...
**Last updated:** 2026-07-10 · **Now at:** Phase 5 + Door-1 diagnostic COMPLETE — captaincy is *largely irreducible on one season*: the best captain is unpredictable ex-ante, the model's divergent picks lose (win-rate 0.41), the only signal (in-form premium, AUC 0.61) is barely above the one-season detectability floor (0.58) → **RESUME HERE: Door 2 (multi-season data)** — the only thing that can power the weak signal / sharpen any decision claim. Injury-aware P(play), squad/team-stacking are downstream of that. Phase 3.2 odds-blocked. `main` clean & pushed.
...
- **3.1 Monte-Carlo simulator ✅ (done — 2026-07-09).** Built `model/forecast/simulator.py`; internal-correctness gate PASSED (sim mean vs analytic `full_pts` corr 0.9988, mad 0.039, n=9147); per-position distributions + P(haul≥10) (GK 7.5% / DEF 3.9% / MID 2.1% / FWD 2.1%); 5 tests. [result](studies/results/predictive-phase3-simulator.md). Design notes below. *Goal:* sample the components through the real scoring rules → full points **distribution** (P(haul≥10), captaincy ceiling p90, downside p10). *Where:* `model/forecast/simulator.py` (new), **consumes `walk_forward_points` output** (recover sampling params from its columns — `λ_goals=e_goals`, `λ_ga=−log(p_cs)`, `p_dc`, `p60`, `e_saves`), reuses `domain/fpl_scoring.py`. *Machinery:* N≈10k seeded draws per player-GW; per draw sample `play60~Bern(p60)`, `goals/assists~Poisson`, `saves~Poisson`, `DC~Bern(p_dc)`, **team `GA~Poisson(λ_ga)` once per team-fixture** → CS`=1{GA=0}·1{≥60'}` and conceded`=−floor(GA/2)` from that one draw; bonus = proxy applied to the drawn returns; sum through the scoring rule.
...
- **Door-1 diagnostic — *why* base_season wins captaincy ✅ (2026-07-10).** `model/eval/captaincy_diagnostics.py` + `captaincy_ceiling_diagnostic.ipynb`. **Largely irreducible on one season:** the oracle captain is unpredictable ex-ante (hit@1 ≈ chance); the model's divergent picks *lose* (win-rate 0.41, −1.44 pts) → its weekly deviations are noise-negative; a weak in-form-premium signal exists (AUC 0.61) but sits **just above the one-season detectability floor (0.58)** → underpowered. **Do not chase captaincy edge with more model machinery; the answer needs Door 2 (seasons).** [results](studies/results/predictive-phase5-captaincy-diagnostic.md).
