...
| A-P1 | **`minutes ≥ 60` "qualified start"** + **proportional-minutes exposure** | Med | minutes-band + Poisson-β test ([results](studies/results/predictive-phase2-overdispersion.md)) | Phase 2 | ✅ **tested-fails** — β<1 for DEF/FWD (offset invalid); short mins carry higher per-90 rate → `minutes≥60` NOT adopted; keep `minutes>0`, minutes enters flexibly |
