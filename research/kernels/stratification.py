...
def quintile_stratification(
    ...
    """Compute quintile-stratified means and a decision-relevance verdict.
    ...
    Args:
        ...
        gap_threshold: Minimum |Q5-Q1| gap required for decision relevance.
                       Use 1.0 for continuous targets; 0.10 for binary [0,1].
        bidirectional: If True, accepts monotone-decreasing as well as
                       monotone-increasing. ``decision_relevant`` uses
                       ``abs(gap)`` rather than raw ``gap``. Default False.

    Returns:
        Dict with quintile means, gap, monotonicity flags, and
        ``decision_relevant``, or None when there are fewer than MIN_N valid
        paired observations or quintile construction fails.
    """
    ...
    try:
        ...
        return {
            ...
            "decision_relevant": bool(effective_gap >= gap_threshold and is_monotonic),
        }
    except Exception:
        return None
