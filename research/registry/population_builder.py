...
def _build_registry_population(
    ...
    required = {"player_id", "gw", "position_code", "minutes", "total_points"} | set(REGISTRY_BUILD_INPUT_COLUMNS)
    ...
    if unmapped:
        ...
        raise ValueError(f"{unmapped} rows have unrecognized position_code values: {bad_codes}")

    duplicates = int(df[["player_id", "gw"]].duplicated().sum())
    if duplicates:
        raise ValueError(f"grain violation: {duplicates} duplicate (player_id, gw) pairs in prepared dataset")
