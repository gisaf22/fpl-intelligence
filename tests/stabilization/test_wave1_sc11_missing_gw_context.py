...
def test_missing_gw_context_raises_immediately():
    ...
    assert "5" in str(exc_info.value), f"Exception message should name GW 5 as missing. Got: {exc_info.value}"
...
def test_missing_gw_context_does_not_return_partial_result():
    ...
    assert result is None, "build_player_gameweek_spine must not return a partial DataFrame when GW context is missing"
