...
def test_staging_sql_contains_order_by_players():
    ...
    assert "ORDER BY" in query, f"players staging query has no ORDER BY — row order is filesystem-defined.\n{query}"
...
def test_first_col_semantics_registry_exists():
    ...
    valid_types = {"invariant_per_gw", "canonical_first_fixture", "temporally_first", "representative_arbitrary"}
    ...
    df = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "position_label": "FWD",
                "team_id": 1,
                "purchase_price": 8.0,  # DIFFERENT — violates invariant_per_gw
                "ownership_count": 1000,
                "transfers_in": 100,
                "transfers_out": 50,
                "transfers_balance": 50,
