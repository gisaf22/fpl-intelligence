...
def test_bgw_null_vs_zero_rolling_divergence():
    ...
    def _roll3_at_gw5(arr: pd.array) -> float:
        return float(pd.Series(arr, dtype="Float64").shift(1).rolling(3, min_periods=1).mean().iloc[4])

    roll3_null = _roll3_at_gw5(points_null)
    roll3_zero = _roll3_at_gw5(points_zero)

    assert abs(roll3_null - 25.0) < 1e-9, f"NULL BGW roll3 expected 25.0, got {roll3_null}"
    assert abs(roll3_zero - (50.0 / 3)) < 1e-9, f"Zero BGW roll3 expected {50.0 / 3:.4f}, got {roll3_zero}"
...
def test_required_input_cols_constant_is_superset_of_roll_cols():
    ...
    assert set(_ROLL_COLS).issubset(_REQUIRED_INPUT_COLS), (
        f"_REQUIRED_INPUT_COLS is missing _ROLL_COLS entries: {set(_ROLL_COLS) - _REQUIRED_INPUT_COLS}"
    )
