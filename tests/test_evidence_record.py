...
from research.families.evidence_record import build_signal_verdict, decision_class_for, write_evidence
...
class TestBuildSignalVerdict:
    ...
    def test_full_corr_fields_projected(self):
        ...
        assert verdict["rho_pooled"] == 0.15
        assert verdict["rho_ci_lower"] == 0.05
        assert verdict["rho_ci_upper"] == 0.25
    ...
    def test_no_full_assoc_fields_are_none(self):
        verdict = build_signal_verdict("xgi_roll3", "FWD", None, [], _qualification("uninformative"))
        assert verdict["rho_pooled"] is None
        assert verdict["rho_ci_lower"] is None
        assert verdict["rho_ci_upper"] is None
