...
def test_get_player_fixture_base_gw_filter():
    df = get_player_fixture_base(DB_PATH, gw=1)
    assert not df.empty
    assert (df["gw"] == 1).all()
...
def test_get_player_fixture_base_grain_violation_raises(monkeypatch):
    ...
    import dal.staging.entities as entities_mod
    from dal.staging.transformer import stage as real_stage
...
def test_get_fixture_context_required_cols_present():
    df = get_fixture_context(DB_PATH)
...
def test_get_fixture_context_gw_filter():
    df = get_fixture_context(DB_PATH, gw=1)
    assert not df.empty
    assert (df["gw"] == 1).all()
