...
def test_xgc_correctness_uses_only_90min_players():
    ...
    assert abs(actual - 1.52) < 1e-6, f"Expected opponent_xgc_roll3=1.52 (90-min only), got {actual}"
...
def test_build_player_opponent_defensive_context_no_look_ahead():
    ...
    for team_id in team_def["team_id"].unique():
        ...
        if 1 in gws_with_data and 2 in gws_with_data and 3 in gws_with_data and 4 in gws_with_data:
            ...
            players_facing = analytics[(analytics["opponent_team_id"] == team_id) & (analytics["gw"] == 4)][
                "player_id"
            ].unique()
            if len(players_facing) == 0:
                continue

            player_row = result[(result["player_id"] == players_facing[0]) & (result["gw"] == 4)]
            if player_row.empty:
                continue

            actual = player_row["opponent_goals_conceded_roll3"].iloc[0]
            assert abs(actual - expected) < 1e-6, (
                f"Look-ahead detected for team {team_id} at GW 4: expected {expected:.4f} (GWs 1-3), got {actual:.4f}"
            )
            return
...
def test_build_player_opponent_defensive_context_dgw_max_aggregation():
    ...
    actual = result[(result["player_id"] == player_id) & (result["gw"] == gw)]["opponent_goals_conceded_roll3"].iloc[0]
...
def test_team_id_resolution_igor_loan():
    ...
    assert gw4_row["team_id"].iloc[0] == 19, f"Igor GW4: expected West Ham (19), got {gw4_row['team_id'].iloc[0]}"

    gw24_row = igor[igor["gw"] == 24]
    assert not gw24_row.empty, "Igor has no GW24 row"
    assert gw24_row["team_id"].iloc[0] == 6, f"Igor GW24: expected Brighton (6), got {gw24_row['team_id'].iloc[0]}"
...
def test_team_id_resolution_guehi_january_transfer():
    ...
    assert gw23_row["team_id"].iloc[0] == 13, f"Guéhi GW23: expected Man City (13), got {gw23_row['team_id'].iloc[0]}"
...
def test_team_id_resolution_brentford_gw26():
    ...
    assert (brentford_gw26["team_id"] == 5).all(), "Not all Brentford GW26 players have team_id == 5"
...
def test_build_player_opponent_defensive_context_dgw_fixture_difficulty_avg():
    ...
    actual = result[(result["player_id"] == player_id) & (result["gw"] == gw)]["fixture_difficulty_avg"].iloc[0]

    assert abs(actual - expected_mean) < 1e-6, f"DGW mean FDR failed: expected {expected_mean:.4f}, got {actual:.4f}"
