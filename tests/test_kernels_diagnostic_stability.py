...
from research.kernels.descriptive.block_distributions import compute_signal_block_distributions
...
# ---------------------------------------------------------------------------
# assess_distribution_stability — all three classifications
# ---------------------------------------------------------------------------
...
def test_classify_stable_when_identical_blocks():
    ...
    assert assess_distribution_stability(stats) == "stable"
...
def test_classify_moderate_shift():
    ...
    result = assess_distribution_stability(stats)
...
def test_classify_unstable_when_large_shift():
    ...
    assert assess_distribution_stability(stats) == "unstable"
...
def test_classify_moderate_shift_boundary_just_above_stable():
    ...
    assert assess_distribution_stability(stats) == "moderate_shift"


def test_classify_raises_on_missing_columns():
    stats = pd.DataFrame({"block": ["b1", "b2"], "median": [1.0, 2.0]})
    with pytest.raises(ValueError, match="missing required columns"):
        assess_distribution_stability(stats)


# ---------------------------------------------------------------------------
# assess_distribution_stability — edge cases
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# assess_distribution_stability — via compute_signal_block_distributions
# ---------------------------------------------------------------------------
...
def test_stable_signal_end_to_end():
    ...
    assert assess_distribution_stability(block_stats) == "stable"
...
def test_unstable_signal_end_to_end():
    ...
    df = _make_df(first_half_values=first, second_half_values=second)
    block_stats = _block_stats_for(df)
    assert assess_distribution_stability(block_stats) == "unstable"
...
def test_pooling_stable_maps_to_pool_confirmed():
    assert resolve_pooling_strategy("stable") == "pool_confirmed"


def test_pooling_moderate_shift_maps_to_pool_with_caveat():
    assert resolve_pooling_strategy("moderate_shift") == "pool_with_caveat"


def test_pooling_unstable_maps_to_restrict_to_midseason():
    assert resolve_pooling_strategy("unstable") == "restrict_to_midseason"
...
def test_pooling_mapping_is_complete():
    ...
    for h in BLOCK_HOMOGENEITY_VALUES:
        decision = resolve_pooling_strategy(h)
        assert decision in POOLING_DECISION_VALUES


def test_pooling_raises_on_unknown_value():
    with pytest.raises(ValueError, match="unrecognized stability verdict"):
        resolve_pooling_strategy("unknown_class")
