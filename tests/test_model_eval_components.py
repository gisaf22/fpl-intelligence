...
def test_score_gates_stacks_and_sorts() -> None:
    ...
    df = pd.DataFrame({"gw": np.repeat(np.arange(1, 7), 20), "position": "MID", "good": rng.normal(size=120)})
...
def test_gate_result_passed_all_requires_both_criteria() -> None:
    ...
    g = TermGateResult(
        term="t",
        table=pd.DataFrame(),
        passed={"DEF": True, "MID": True},
        calibration=pd.DataFrame(),
        passed_calibration={"DEF": False, "MID": True},
    )
