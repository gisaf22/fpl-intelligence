...
def test_full_points_composition_and_gate() -> None:
    df = walk_forward_points(_panel(seed=11))
    scored = df.dropna(subset=["full_pts"])
    assert np.isfinite(scored["full_pts"]).all()        # composed E[points] is finite
    assert "p21_pts" in df.columns and "base_season" in df.columns
    res = points_model_gate(_panel(seed=11))
    assert set(res.index.get_level_values("model")) == {
        "full points model", "Phase-2.1 component model", "base_season (incumbent)"}
    assert res["spearman"].dropna().between(-1, 1).all()


def test_full_points_leakage_safe_shape() -> None:
    # base_season is a strictly-prior expanding mean -> a player's first row is NaN (no leakage)
    df = walk_forward_points(_panel(seed=12))
    first = df.sort_values("gw").groupby("player_id").head(1)
    assert first["base_season"].isna().all()
