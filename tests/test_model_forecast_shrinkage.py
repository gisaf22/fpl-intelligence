...
def test_lambda_grows_with_history() -> None:
    ...
    d["closeness"] = 1 - (d["lvl_shrunk"] - d["lvl_mean"]).abs() / ((d["mu_pos"] - d["lvl_mean"]).abs() + 1e-9)
...
def test_variance_ratio_infinite_when_no_between_signal() -> None:
    ...
    rows = [{"player_id": p, "total_points": 5.0 + rng.normal(0, 2.0)} for p in range(20) for _ in range(10)]
