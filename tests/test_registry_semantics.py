...
def test_enrich_signal_layers_replaces_stale_semantic_columns():
    ...
    assert enriched.loc[enriched["signal"].eq("fdr_avg"), "signal_layer"].eq("context").all()
    assert enriched.loc[enriched["signal"].eq("fdr_avg"), "downstream_status"].eq("caveated").all()
