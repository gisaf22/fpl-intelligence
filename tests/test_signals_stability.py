...
def _make_df(
    ...
    rows = (
        [{gw_column: gw, "position": position, signal_name: v}
         for gw, v in zip(first_gws, first_half_values)]
        + [{gw_column: gw, "position": position, signal_name: v}
           for gw, v in zip(second_gws, second_half_values)]
    )
...
def test_custom_gw_blocks_are_respected():
    ...
    result = compute_signal_block_distributions(
        df, signals=["xg"], positions=["MID"], gw_blocks=custom_blocks
    )
...
def test_custom_gw_column_name():
    ...
    result = compute_signal_block_distributions(
        df, signals=["xg"], positions=["MID"],
        gw_column="gameweek", gw_blocks=blocks,
    )
    assert len(result) == 2
