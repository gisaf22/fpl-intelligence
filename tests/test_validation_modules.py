...
class TestValidateRowCompleteness:
    ...
    def test_missing_one_pair_fails(self):
        players = [1, 2]
        gws = [1, 2, 3]
        rows = [{"player_id": p, "gw": g} for p in players for g in gws if not (p == 1 and g == 2)]
        df = pd.DataFrame(rows)
        with pytest.raises(DALContractViolation) as exc_info:
...
class TestValidateNoFutureData:
    ...
    def test_future_rows_all_null_passes(self):
        ...
        df = self._base_df()
        assert validate_no_future_data(df, reference_gw=20, performance_cols=self._PERF_COLS) is None
