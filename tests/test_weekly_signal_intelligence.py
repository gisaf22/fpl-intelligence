...
def test_positional_summary_blocked_count_matches_registry(positional_summary, registry):
    expected_blocked = registry[registry["downstream_status"] == "blocked"].groupby("position").size()
...
def _all_text(df: pd.DataFrame) -> str:
    ...
    return " ".join(df.select_dtypes(include="object").fillna("").values.flatten().tolist()).lower()


def test_stable_observations_contains_no_forbidden_phrases(stable_obs):
    text = _all_text(stable_obs)
    for phrase in FORBIDDEN_PHRASES:
        assert phrase not in text, f"forbidden phrase {phrase!r} found in stable_signal_observations"


def test_positional_summary_contains_no_forbidden_phrases(positional_summary):
    text = _all_text(positional_summary)
    for phrase in FORBIDDEN_PHRASES:
        assert phrase not in text, f"forbidden phrase {phrase!r} found in positional_signal_summary"


def test_context_notes_contains_no_forbidden_phrases(context_notes):
    text = _all_text(context_notes)
    for phrase in FORBIDDEN_PHRASES:
        assert phrase not in text, f"forbidden phrase {phrase!r} found in context_condition_notes"
...
def test_exposure_note_template_differs_from_context_note():
    context_row = _make_registry_row(
        signal="was_home", position="MID", signal_layer="context", promotion_class=None, downstream_status="caveated"
    )
    context_row["promotion_class"] = None
    exposure_row = _make_registry_row(
        signal="minutes", position="MID", signal_layer="exposure", promotion_class=None, downstream_status="caveated"
    )
